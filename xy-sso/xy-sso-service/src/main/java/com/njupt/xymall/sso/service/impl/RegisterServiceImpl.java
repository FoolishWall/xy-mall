...
public class RegisterServiceImpl implements RegisterService {
	...
	@Override
	public XYResult checkData(String param, int type) {
		// 根据不同的type生成不同的查询条件
		...
		if (type == 1) {
			...
			return XYResult.build(400, "数据类型错误");
		}
		// 执行查询
		...
		if (list != null && list.size() > 0) {
			// 如果有数据返回false
			return XYResult.ok(false);
		}
		// 如果没有数据返回true
		return XYResult.ok(true);
	}
	...
	public XYResult register(TbUser user) {
		...
		if (StringUtils.isBlank(user.getUsername()) || StringUtils.isBlank(user.getPassword())
			...
			return XYResult.build(400, "用户数据不完整，注册失败");
		}
		// 1：用户名 2：手机号 3：邮箱
		XYResult result = checkData(user.getUsername(), 1);
		if (!(boolean) result.getData()) {
			return XYResult.build(400, "此用户名已经被占用");
		}
		result = checkData(user.getPhone(), 2);
		if (!(boolean) result.getData()) {
			return XYResult.build(400, "手机号已经被占用");
		}
		// 补全pojo的属性
